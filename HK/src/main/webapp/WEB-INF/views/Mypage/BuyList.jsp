<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>Insert title here</title>
</head>
<body><div id="MypageMaintitle" >구매내역</div>
	<table>
		<tr>
			<td align="center">
				<table id="BuyperiodTable">
					<tr>
						<td><div id="Buyperioddiv">조회기간</div></td>
						<td>
							<div class="Buyperiodclick">1개월</div>
						</td>
						<td>
							<div class="Buyperiodclick">3개월</div>
						</td>
						<td>
							<div class="Buyperiodclick">6개월</div>
						</td>
						<td id="widthtd"></td>
						<td>
						<input>
						</td>
						<td>
						<div class="Buyperiodcalendardiv">
							<img src="resources/img/calendar.png">
						</div>
						</td>
						<td id="swung">~</td>
						<td>
						<input>
						</td>
						<td><div class="Buyperiodcalendardiv">
							<img src="resources/img/calendar.png">
						</div>
						</td>
						<td><button>조회</button></td>
					
					</tr>
				</table>

			</td>
		
		<tr>
			<td>
				<table id="MypageBuyTable">
					<tr>
						<td class="Buydaytd">날짜</td>
						<td class="Buyproducttd">상품정보</td>
						<td class="Buypricetd">가격</td>
						<td class="Buystatetd">상태</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-01</td>
						<td class="Buyproducttd2">애플 에어팟</td>
						<td class="Buypricetd2">190$</td>
						<td class="Buystatetd2">수령후</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-05</td>
						<td class="Buyproducttd2">에어프라이어</td>
						<td class="Buypricetd2">90$</td>
						<td class="Buystatetd2">수령전</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-10</td>
						<td class="Buyproducttd2">에어드라이어</td>
						<td class="Buypricetd2">1000$</td>
						<td class="Buystatetd2">수령전</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-10</td>
						<td class="Buyproducttd2">에어드라이어</td>
						<td class="Buypricetd2">1000$</td>
						<td class="Buystatetd2">수령전</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-10</td>
						<td class="Buyproducttd2">에어드라이어</td>
						<td class="Buypricetd2">1000$</td>
						<td class="Buystatetd2">수령전</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-10</td>
						<td class="Buyproducttd2">에어드라이어</td>
						<td class="Buypricetd2">1000$</td>
						<td class="Buystatetd2">수령전</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-10</td>
						<td class="Buyproducttd2">에어드라이어</td>
						<td class="Buypricetd2">1000$</td>
						<td class="Buystatetd2">수령전</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-10</td>
						<td class="Buyproducttd2">에어드라이어</td>
						<td class="Buypricetd2">1000$</td>
						<td class="Buystatetd2">수령전</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-10</td>
						<td class="Buyproducttd2">에어드라이어</td>
						<td class="Buypricetd2">1000$</td>
						<td class="Buystatetd2">수령전</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-10</td>
						<td class="Buyproducttd2">에어드라이어</td>
						<td class="Buypricetd2">1000$</td>
						<td class="Buystatetd2">수령전</td>
					</tr>
					<tr>
						<td class="Buydaytd2">2019-02-10</td>
						<td class="Buyproducttd2">에어드라이어</td>
						<td class="Buypricetd2">1000$</td>
						<td class="Buystatetd2">수령전</td>
					</tr>
				</table>
			</td>
		</tr>
	</table>

</body>
</html>